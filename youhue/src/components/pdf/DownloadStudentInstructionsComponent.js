...
import "../../styles/pdf/studentInstructions.css";
import { Link } from "react-router-dom";

class DownloadStudentInstructionsComponent extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      access_code: "",
  ...
  componentDidMount() {
    ...
    fetch(baseUrl+"v5/email-instruction/", {
      method: "POST",
      headers: {
        Authorization: token,
      ...
      .then((response) => {
        ...
        if (response.status) {
          ...
          this.setState({
            ...
            identifier: response.data.group_obj,
            access_code: response.data.student,
          });
          setTimeout(() => {
            window.print();
          }, 1500);
  ...
  render() {
    const { access_code, qr_code, identifier } = this.state;
...
export default DownloadStudentInstructionsComponent;
