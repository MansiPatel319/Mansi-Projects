...
import CreatorOneToOneEarningComponent from '../../components/Creator/MyEarningComponent/CreatorOneToOneEarningComponent';
import { useHistory } from 'react-router-dom';
import FooterComponent from '../../components/Footer/FooterComponent';

function CreatorOneToOneEarning() {
  const history = useHistory();
  useEffect(() => {
    const checkCreator = localStorage.getItem('is_creator');
    if (checkCreator === 'true') {
      history.push('/creator-one-to-one-earnings');
    } else {
          ...
          <div className="header-footer-with-min-height02">
            <div className="pattern-inner-div pattern-upricing-inner-div">
              <div className="relative-index">
                <CreatorOneToOneEarningComponent />
            </div>
          </div>
        </div>
        </div>
...
export default CreatorOneToOneEarning;
